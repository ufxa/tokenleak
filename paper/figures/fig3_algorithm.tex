% Algorithm 1: TokenLeak (double-column float; caption above)
\begin{algorithm*}[t]
\caption{TokenLeak: Three-Phase Prompt Reconstruction via Side-Channel Fusion}
\label{alg:tokenleak}
\DontPrintSemicolon
\SetAlgoLined
...
\BlankLine
\textbf{Complexity Analysis:} Profiling: $\mathcal{O}(|\mathcal{V}| \cdot m)$; Online observation: $\mathcal{O}(n)$;\\
\hphantom{\textbf{Complexity Analysis:}} Reconstruction: $\mathcal{O}(n \cdot |\mathcal{V}|)$ per query.
\end{algorithm*}
