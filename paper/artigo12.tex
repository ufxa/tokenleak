...
projects lack resources for custom defense development, creating a two-tiered security
landscape where prompt confidentiality depends on organizational scale.
...
Per-token accuracy is non-uniform across vocabulary: high-frequency function words (``the'', ``of'', ``a'') achieve near-100\% accuracy due to distinctive embedding positions, while low-frequency technical tokens suffer from profile sparsity and achieve only 40--55\% accuracy. This vocabulary skew implies that practical information leakage risk is higher for natural-language prompts (dense with high-frequency tokens) than for code prompts (dense with rare identifiers).

The cache-timing fusion provides substantial orthogonal signal: disabling either channel independently reduces accuracy by 22.1~pp (removing cache) or 21.6~pp (removing timing). This synergy motivates defenses targeting both channels: random delay alone (58.2\% Top-1) or cache oblivion alone (51.6\%) leaves the complementary channel partially intact, whereas combined defenses approach random-guess performance (18.7\% with DP+cache-oblivion).
