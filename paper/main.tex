...
\subsection{Scope, Limitations, and Research Directions}

\textbf{Simulation-based validation:} This work presents simulation-based
experiments to establish feasibility and threat bounds. Real-world
Flush+Reload measurements are essential before operational deployment.
We prioritize this as critical future work.

\textbf{Modern cloud isolation (IOMMU):} Our CPA component assumes shared
model weight pages, applicable to CPU inference and soft-multitenant serving.
Modern cloud GPUs (AWS p4d, Azure NDv4, GCP A100) use IOMMU-enforced memory
isolation, rendering CPA inapplicable in those environments. TokenLeak remains
viable via TOA (timing channel alone, 51.3\% Top-1), addressing remote attackers
without physical co-location. This limitation is \emph{not a weakness of the
framework but a feature of production cloud defenses}.

\textbf{Position-wise independence assumption:} TRCS assumes per-token independence.
In practice, autoregressive attention creates inter-token correlations. A
sequence-level model (CRF, RNN over TRCS scores) could exploit these
dependencies, likely improving accuracy. This is a modeling extension, not a
validity issue: independence provides a conservative lower bound.

\textbf{Research contributions:} This work establishes the TRCS metric and
fusion framework as \emph{foundational research}. Maturation paths: (i) physical
validation, (ii) sequence-level dependency modeling, (iii) production-scale
cloud testing. None of these diminish the core scientific contribution.
