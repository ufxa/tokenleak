...
of a victim process~\cite{yarom2014flush,hong2023sok}.
...
cache sets~\cite{liu2015last}. Hong et al.~\cite{hong2023sok}
provide a systematic evaluation of these techniques against
GPU-accelerated deep learning workloads. Ramesh et
al.~\cite{ramesh2024profiling} extended Flush+Reload to cloud GPU
inference services, demonstrating cross-VM attacks in production
deployments. These vectors directly inform the CPA component in
TokenLeak.
...
Wen et al.~\cite{wen2024side} systematically characterized
network-timing side channels against LLM inference endpoints,
directly analogous to our TOA network-level probe.
...
Hong et al.~\cite{hong2023sok} provide a comprehensive
evaluation of GPU side-channel attacks spanning power analysis,
cache, and memory bus snooping against deep learning workloads.
Stoian et al.~\cite{stoian2024cachegrind} extended cache-based
techniques to transformer inference workloads, demonstrating
architecture inference via LLC profiling.
Ramesh et al.~\cite{ramesh2024profiling} demonstrated GPU
performance counter attacks in cloud AI services, enabling
cross-VM timing inference. Our work targets a distinct goal: not
model extraction, but prompt content reconstruction.

\subsection{LLM Privacy and Training Data Attacks}

Carlini et al.~\cite{carlini2021extracting} and Nasr et
al.~\cite{nasr2023scalable} demonstrated that training data can be
extracted from language models via black-box API access. Nasr et
al.~\cite{nasr2023scalable} scaled such extraction to production
systems. Patil et al.~\cite{patil2024review} systematically surveyed
information leakage and privacy risks in LLM deployments.
Zhou et al.~\cite{zhou2024privacyllm} provided a comprehensive
analysis of privacy-preserving inference for large language models.
...
das2024security} provides formal guarantees but degrades
model utility. Trusted execution environments~\cite{shen2024confidential}
isolate computation but incur significant overhead and do not
prevent timing side channels at service level.
Oblivious inference~\cite{shen2024confidential} masks memory access
patterns but is computationally prohibitive for billion-parameter
models.
...
\resizebox{\columnwidth}{!}{\input{figures/fig6_scalability}}
\end{figure}
